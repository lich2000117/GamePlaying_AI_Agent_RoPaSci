...
from RL_train.util import action_throw, move_action, least_distance, get_six_adj_nodes, is_point_valid

def get_all_valid_action(state, player):
    # state =  (play_dict, player's throws left, opponnet's throws left, player's side)
    ...
    if player == "player":
        if state.throws_left > 0:
            throw_list = get_all_valid_throw(state, player)
            if throw_list:
...
def get_all_valid_throw(state, whichPlayer = "player"):
    ...
    if whichPlayer == "player":
        if state.side == "upper":
            ...
            # print("Cannont throw")
            return []
    elif whichPlayer == "opponent":
        if state.side == "upper":
            # then our opponent is the lower
            for row in range(-4, -3 + enemy_have_thrown):
                for col in range(-4, 5):
                    if abs(row + col) <= 4:
                        throw_list.append((row, col))
            return throw_list
        elif state.side == "lower":
            for row in range(4, 3 - enemy_have_thrown, -1):
                for col in range(-4, 5):
                    if abs(row + col) <= 4:
                        throw_list.append((row, col))
            return throw_list
