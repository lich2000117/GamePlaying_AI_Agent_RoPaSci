...
def calculate_normal_probability(value, array):
    '''
    ...
    if not std:
        ...
        prob = scipy.stats.norm(np.mean(array), std).cdf(value)
        prob = abs(prob-(1-prob))
        return 1-prob
...
def closest_one(target_pos, candidate_list):
    ...
    for candidate in candidate_list:
        distance_list.append((least_distance(candidate, target_pos), candidate))
