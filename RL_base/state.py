...
def isGameEnded(state, ourPlayer, opponent):
    num_tokens = len(state.play_dict[ourPlayer]['r'] + \
                    state.play_dict[ourPlayer]['p'] + state.play_dict[ourPlayer]['s'])
    ...
    if num_throws == 0:
        for type in ['r', 'p', 's']:
            ...
            if state.play_dict[opponent][type]:
                ...
                if (len(state.play_dict[ourPlayer][counter_type]) + len(state.play_dict[opponent][counter_type])) == 0:
        ...
        for type in ['r', 'p', 's']:
            ...
            if state.play_dict[ourPlayer][type]:
                # counter type will be 'r'
                counter_type = target_dict[target_dict[type]]
                if (len(state.play_dict[opponent][counter_type]) + len(state.play_dict[ourPlayer][counter_type])) == 0:
